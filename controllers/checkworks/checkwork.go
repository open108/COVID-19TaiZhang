...
func (this *ManageCheckworkAllController) Get() {
	//权限检测
	if !strings.Contains(this.GetSession("userPermission").(string), "clinicmanage") {
		this.Abort("401")
	}
	...
	condArr := make(map[string]string)
	condArr["date"] = date
	condArr["shopid"] = fmt.Sprintf("%d", pro.ShopID)

	//yanji211
	countCheckworks, _, checkworks := ListYQTZAll(condArr)
	fmt.Println(checkworks)
	this.Data["condArr"] = condArr
	this.Data["checkworks"] = checkworks
	this.Data["ShopName"] = pro.ShopName
	// this.Data["year"] = time.Now().Format("2006")
	// this.Data["month"] = time.Now().Format("1")
