$(function(){
	...
	*/
	...
	$('#taizhangprofile-form').validate({
		ignore: '',
		rules: {
			pfgaozhi: { required: true },
			uname: { required: true },
			birth: { required: true },
			uID: { required: true },
			uaddr: { required: true },
			uprofession: { required: true },
			utelphone: { required: true },
			uqtelphone: { required: true }
		},
		messages: {
			pfgaozhi: { required: '请选择是否告知' },
			uname: { required: '请填写您的名字' },
			birth: { required: '请填写您的生日' },
			uID: { required: '请填写您的身份证号' },
			uaddr: { required: '请填写您的详细住址' },
			uprofession: { required: '请填写您的职业' },
			utelphone: { required: '请输入您的手机号' },
			uqtelphone: { required: '请填写您亲属的手机号' }
		},
		submitHandler: function (form) {
			$(form).ajaxSubmit({
				type: 'POST',
...
function dialogInfo(msg) {
    ...
    html += '<h4 class="modal-title" id="dialogInfoTitle">温馨提示：</h4>';
    html += ' </div>';
